...
class DestinatariNEW extends Widget {
    ...
    public function run() {
        ...
        $networks = CwhNodi::find()
            ...
            ->all(); //GROUP BY PER CWH CONFIG ID O AND WHERE TIPO RETE
        ...
        foreach ($networkModels as $networkModel) {
            ...
            foreach ($networks as $network) {
                if ($scopeFilter) {
                    ...
                    if ($enableDestinatariFatherChildren && isset($this->scope[$networkModel->tablename]) && ($network->record_id == $this->scope[$networkModel->tablename] || $this->isNetworkChild($network->record_id, $children) || $this->isNetworkFather($network->record_id, $this->scope[$networkModel->tablename], $networkModel) || $this->isNetworkBrother($network->record_id, $this->scope[$networkModel->tablename], $networkModel))) {
                        $destinatari[] = $network;
                    }
                } else {
            ...
            if (isset($networkIds[$networkModel->classname])) {
                $rows = $networkObject->getListOfRecipients(
                    ...
                    $userIds[$networkModel->classname]
                );

                $destinatariForScope[$categoryName] = $destinatariData[$categoryName] = ArrayHelper::map($rows, 'objID', 'name');
            }
        }
    ...
    public function renderWithoutScope($destinatariData, $labelSuffix) {
        $primo_piano_checkbox = Yii::$app->user->can('UROI_LANDING') ?
            (Yii::$app->getModule('news')->params['site_publish_enabled'] ? Html::checkbox('landing-checkbox', $this->model->regola_pubblicazione == 5, ['label' => AmosCwh::t('message', 'Proponi pubblicazione sul portale pubblico'), 'id' => 'landing-checkbox']) : '') : '';
...
//                        'value' => $scopeFilter ? $destinatari[0]['id'] : null,
    ...
    public function renderWithScope($destinatariForScope, $labelSuffix) {
        ...
        $destinatariField->hiddenInput(['value' => (isset($destinatariForScope[0]['id']) ? $destinatariForScope[0]['id'] : null), 'id' => 'cwh-destinatari', 'name' => $this->getNameField() . '[destinatari][]']);
